describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
     ...
     */
      ...
      const shadowRoot = await productItem.evaluateHandle(element => element.shadowRoot);
      ...
      await button.click();
      text = await (await button.getProperty('innerText')).jsonValue();
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (const item of prodItems) {
      const shadow = await item.evaluateHandle(element => element.shadowRoot);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (const element of prodItems) {
      const shadowRoot = await element.evaluateHandle(element => element.shadowRoot);
      const button = await shadowRoot.asElement().$('button');
      const text = await (await button.getProperty('innerText')).jsonValue();
      expect(text).toBe('Add to Cart');
    }
